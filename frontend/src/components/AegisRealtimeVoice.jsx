...
export default function AegisRealtimeVoice({token}){
 ...
 const connectionRef=useRef(null),busyRef=useRef(false),ctxRef=useRef(null),sourceRef=useRef(null),requestRef=useRef(null),audioRef=useRef(null),audioUnlockedRef=useRef(false),readerRef=useRef(null),playbackGenerationRef=useRef(0);
 ...
 const stopAudio=()=>{playbackGenerationRef.current+=1;requestRef.current?.abort?.();requestRef.current=null;try{readerRef.current?.cancel?.()}catch{}readerRef.current=null;try{sourceRef.current?.stop()}catch{}sourceRef.current=null;try{const a=audioRef.current;if(a){a.pause();a.removeAttribute('src');a.load();a.currentTime=0}}catch{}try{window.speechSynthesis?.cancel()}catch{}setState(s=>['speaking','preparing'].includes(s)?'idle':s)};
