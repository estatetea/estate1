...
export async function POST(request,{params}){
 ...
 const b=report.owner_brief||{},n=b.numbers||{},sections=b.agent_sections||[];
 const cards=[
 ['Activities',n.total_agent_activities??0],['Completed',n.completed_agent_activities??0],
 ['Approvals',n.pending_approvals??0],['Open issues',n.open_issues??0],
 ['Handoffs',n.pending_handoffs??0],['Awaiting payment',n.awaiting_payment??0],
 ['Samples',n.sample_requests??0],['Orders',n.order_requests??0]
 ];
 const agents=sections.map(a=>`<section class="agent"><div class="agenthead"><h2>${esc(a.agent)}</h2><span>${esc(a.completed_count??0)} completed · ${esc(a.activity_count??0)} activities</span></div><p class="focus"><b>Today’s focus:</b> ${esc(a.focus||'No active task')}</p><ol>${(a.work||[]).length?(a.work||[]).map(w=>`<li><b>${esc(w.summary)}</b>${w.status?` <span class="status">· ${esc(w.status)}</span>`:''}</li>`).join(''):'<li>No recorded work for this day.</li>'}</ol></section>`).join('');
 const html=`<!doctype html><html><head><meta charset="utf-8"><title>${esc(b.headline||'Estate Tea Aegis Report')}</title><style>@page{size:A4;margin:14mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#191919;margin:0;background:#fff}header{border-bottom:3px solid #b88a34;padding-bottom:16px;margin-bottom:18px}.brand{font-size:12px;letter-spacing:3px;text-transform:uppercase;color:#9a722c}h1{font-size:28px;margin:8px 0 5px}.meta{font-size:11px;color:#777}.summary{font-size:14px;line-height:1.6;background:#f8f5ee;border-left:4px solid #b88a34;padding:14px;margin:16px 0}.grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:16px 0}.card{border:1px solid #ddd;border-radius:8px;padding:10px}.card small{display:block;color:#777;text-transform:uppercase;font-size:9px}.card strong{font-size:21px;color:#9a722c}.needs{border:1px solid #e5d7b8;border-radius:10px;padding:14px;margin:18px 0}.needs h2,.agent h2{margin:0}.agent{break-inside:avoid;border-top:1px solid #ddd;padding:16px 0}.agenthead{display:flex;justify-content:space-between;align-items:baseline}.agenthead span{font-size:11px;color:#777}.focus{font-size:12px;color:#555}ol{padding-left:22px}li{margin:7px 0;font-size:12px;line-height:1.45}.status{color:#777;font-weight:normal}footer{margin-top:20px;border-top:1px solid #ddd;padding-top:10px;font-size:10px;color:#888}@media print{.no-print{display:none}}</style></head><body><header><div class="brand">Estate Tea · Aegis</div><h1>${esc(b.headline||'End-of-day report')}</h1><div class="meta">Report ${esc(report.report_id)} · ${esc(report.created_at)}</div></header><div class="summary">${esc(b.plain_summary||'Daily operations report.')}</div><div class="grid">${cards.map(([l,v])=>`<div class="card"><small>${esc(l)}</small><strong>${esc(v)}</strong></div>`).join('')}</div><section class="needs"><h2>What needs you</h2><ul>${(b.what_needs_you||['Nothing needs your immediate attention.']).map(x=>`<li>${esc(x)}</li>`).join('')}</ul></section><h1>Who did what</h1>${agents||'<p>No agent detail was stored in this older report.</p>'}<footer>Generated by Aegis for Estate Tea. Use your browser’s Print / Save as PDF option if you want a PDF copy.</footer></body></html>`;
